...
| `duplex_plugin` | Dotted path of the model's optional `DuplexModelPlugin`. The effective deploy configuration must also set `session_mode: duplex` before `vllm-omni serve` selects `DuplexOmni`. |
...
| `session_mode` | str | optional | `"turn"` | Session behavior. A pipeline with a `duplex_plugin` is served through `DuplexOmni` only when the effective deploy configuration sets `"duplex"`; otherwise it keeps the ordinary turn-based server. |
