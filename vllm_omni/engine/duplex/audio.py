...
def resample_pcm_f32le_mono(raw: bytes, *, source_rate_hz: int, target_rate_hz: int) -> bytes:
    if source_rate_hz <= 0 or target_rate_hz <= 0 or source_rate_hz == target_rate_hz:
        return raw
    ...
    if reduced_factor > StreamingAudioResampler.max_polyphase_factor:
        ...
        source_x = np.linspace(0.0, 1.0, num=samples.size, endpoint=True)
        target_x = np.linspace(0.0, 1.0, num=target_size, endpoint=True)
        return np.interp(target_x, source_x, samples).astype("<f4").tobytes()
    resampler = StreamingAudioResampler(source_rate_hz, target_rate_hz)
...
def convert_input_audio_with_rate(
    ...
    *,
    ...
    if normalized == "g711_ulaw":
        ...
        if not isinstance(sample_rate_hz, int | float) or int(sample_rate_hz) == target_sample_rate_hz:
            return audio, fmt, sample_rate_hz
        if len(raw) % 4:
            return audio, fmt, sample_rate_hz
        raw = resample_pcm_f32le_mono(
            raw,
            source_rate_hz=int(sample_rate_hz),
            target_rate_hz=target_sample_rate_hz,
        )
        return base64.b64encode(raw).decode("ascii"), "pcm_f32le", target_sample_rate_hz
    elif len(raw) % 2:
        return audio, fmt, sample_rate_hz
